<template>
  <div>
    <div class="ap interactive" @click="mouseClick()">
      ...
      <div class="flex-grow">
        <ProgressBar :fills="fills" :left="false">
...
export default {
  ...
  subscriptions() {
    ...
    return {
      ...
      consideredAP: Rx.combineLatest(consideredAPStream, APStream).map(
        ([consideredAP, AP]) => {
      ...
      AP: APStream.map((value) => Math.floor(value / 60))
        ...
        .tap((value) => {
          if (
            this.consideredAP &&
            this.AP < this.consideredAP &&
            value >= this.consideredAP
          ) {
  ...
  computed: {
    ...
    consideredStyle() {
      ...
      const right =
        100 - Math.min(100, Math.max(minWidth, currentRatio, consideredRatio));
