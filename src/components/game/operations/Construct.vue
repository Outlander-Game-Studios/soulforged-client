    ...
    <Vertical>
      ...
      >
        ...
        <Vertical tight v-if="step === 1">
          <div
            v-for="(material, idx) in structure.materials"
            :key="'material' + idx"
          >
            <Horizontal>
        ...
        >
          <Vertical>
            ...
            <HorizontalCenter>
              <Button
                @click="finishAddingMaterial()"
                :disabled="!addMaterialAmount"
                ref="submit"
              >
                Supply
              </Button>
      ...
      >
        ...
        <Vertical v-if="step === 2">
          ...
          <ProgressBar
            :fills="{
              green:
                (structure.constructionProgress * 100) / CONSTRUCT_RESOLUTION,
...
<script>
export default window.OperationConstruct = {
  props: {
    operation: {},
  },
  ...
  computed: {
    ...
    maxAmount() {
      ...
      return Math.max(
        1,
        Math.floor(this.currentAP / this.operation.context.unitCost)
      );
    },
  },
  ...
  methods: {
    ...
    updateConsideredAP() {
      ControlsService.updateConsideredAP(
        this.workAmount * this.operation.context.unitCost
      );
    },
    ...
    finishAddingMaterial() {
      this.addMaterialRequest(
        this.addingMaterial.publicId,
        this.addMaterialAmount
      ).then(() => {
