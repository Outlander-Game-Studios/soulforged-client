  ...
  <div>
    ...
    >
      ...
      >
        ...
        >
          <RichText class="nowrap" :value="action.label ? action.label : 'Missing label'" />
        </Button>
      </div>
    </component>
    <Modal dialog large v-if="currentAction" @close="closeActionDialog()">
      <template v-slot:title>
        <RichText :value="currentAction.label ? currentAction.label : 'Missing label'" />
      </template>
...
export default rxComponent({
  ...
  computed: {
    filteredActions() {
      ...
      const onlyAction = this.target.actions.find((a) => a.actionId === this.actionId)
      if (!onlyAction) {
        return []
      }
      return [onlyAction]
  ...
  methods: {
    ...
    performAction(action) {
      ...
      this.performing = GameService.performAction(this.target, action, parameters).then(
        (response) => {
