...
export const GameService = (window.GameService = {
  ...
  getOpenPromise(reset = false, force = false) {
    ...
    if (!openPromise) {
      ...
      openPromise = new Promise((resolve, reject) => {
        ...
        GameService.registerHandler(REQUEST_CODES.RE_FETCH_OPEN_QUESTIONS, () => {
  ...
  getCraftDetails(craftId) {
    if (!craftDetailsStreams[craftId]) {
      ...
      if (!cached) {
        ...
        setTimeout(async () => {
          if (
            await icons.someAsync(async (icon) => !(await LocalStorageService.isImageCached(icon)))
          ) {
  ...
  getUtilitiesStream() {
    return GameService.getRootEntityStream()
      .pluck('equipment')
      .switchMap((equipment) => {
        const utils = Object.keys(equipment).filter((slotName) => slotName.includes('Utility'))
        if (!utils.length) {
  ...
  getRootEntityStream(forceReFetchId = false, forceReFetchData = false) {
    ...
    return rootEntityIdStream.distinctUntilChanged().switchMap((rootEntityId) => {
      if (rootEntityId) {
        return GameService.getEntityStream(rootEntityId, ENTITY_VARIANTS.SELF).filter((entity) => {
          if (!entity) {
  ...
  getStructuresIdsStream(includePaths = true) {
    ...
    const pathsStructures = includePaths
      ? locationStream
          ...
          .map((paths) => paths.reduce((acc, p) => [...acc, ...(p.structures || [])], []))
      : Rx.Observable.of([])
  ...
  getItemCountsByPublicIdStream() {
    if (!itemCountStream) {
      itemCountStream = GameService.getInventoryStream(GameService.getRootEntityStream())
        .map((items) => {
          return items
            .filter((i) => !!i)
            .filter((i) => !i.isRuined)
  ...
  checkQuickActions() {
    this.getQuickActionsStream()
      ...
      .subscribe((quickActions) => {
        const includedPublicIds = quickActions.toObject((action) => action.publicId)
  ...
  getQuickActionsStream() {
    ...
    const quickActionsStream = ControlsService.getSettingStream('quickActions', [])
    ...
    return Rx.combineLatest(quickActionsStream, inventoryStream, structuresStream)
      .debounceTime(50)
      .map(([quickActions, inventoryItems, structures]) => {
        const quickActionsMap = quickActions.toObject(getKey, (quickAction) => ({ ...quickAction }))
        ;[...inventoryItems, ...structures].forEach((entity) => {
          entity.actions.forEach((action) => {
            ...
            if (quickActionsMap[publicIdKey]) {
              if (isBetterFitThan(entity, quickActionsMap[publicIdKey].entity)) {
  ...
  getAllItemsByIdStream() {
    return Rx.combineLatest([
      ...
      .map(([bagsItems, locationItems]) => [...(bagsItems || []), ...(locationItems || [])])
  ...
  getEntityStream(
    ...
    if (initial || forceFetch) {
      ...
      GameService.request(REQUEST_CODES.ENTITY_SUB, { payloadId }).then((data) => {
        if (data) {
          entityStreams[payloadId]?.next({
            ...data,
            updatedOn: new Date(),
  ...
  getEntitiesStream(entitiesIds, variant = ENTITY_VARIANTS.BASE, forceFetch = false) {
    if (!entitiesIds) {
      return Rx.Observable.empty()
    }
  ...
  requestPowersInfo() {
    return GameService.request(REQUEST_CODES.GET_POWERS_INFO).then((response) => {
  ...
  getCombatFramesStream() {
    if (!combatFramesStream) {
      combatFramesStream = new Rx.Subject()
      GameService.registerHandler(REQUEST_CODES.COMBAT_FRAME, (frame) => {
        combatFramesStream.next(frame)
      })
    }
    return combatFramesStream
  },
