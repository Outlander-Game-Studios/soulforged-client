...
export const ControlsService = (window.ControlsService = {
  ...
  initiateCordovaLogin() {
    window.webkit.messageHandlers.cordova_iab.postMessage(JSON.stringify({ action: 'login' }))
  },
  ...
  getControlEventStream(expectedEvent) {
    return controlStream.filter(({ event }) => event === expectedEvent).map(({ params }) => params)
  },
  ...
  getDraggedItemStream() {
    if (!draggedItemStream) {
      draggedItemStream = ControlsService.getControlEventStream('draggingItem')
        .map(([item]) => item)
        .switchMap((item) =>
          GameService.getEntityStream(item.id, ENTITY_VARIANTS.BASE, true).tap(null, null, () => {
  ...
  getSettingStream(key, defaultValue) {
    if (!settingStreams) {
      settingStreams = {}
      GameService.registerHandler(REQUEST_CODES.SETTING_UPDATE, ({ settingKey, value }) => {
  ...
  async getSetting(key, defaultValue = undefined) {
    return await ControlsService.getSettingStream(key, defaultValue).first().toPromise()
  },
